...
class DoubleConv(nn.Module):
    """(DepthwiseSeparableConv => [BN] => ReLU) * 2"""

    def __init__(self, in_channels, out_channels, mid_channels=None):
        """
        Initializes the DoubleConv module with depthwise separable convolutions.
...
class OutConv(nn.Module):
    """Output convolution layer with depthwise separable convolution."""

    def __init__(self, in_channels, out_channels):
        """
        Initializes the OutConv module with depthwise separable convolutions.
        ...
        self.conv = nn.Sequential(
            DepthwiseSeparableConv(in_channels, out_channels, kernel_size=1, padding=0, bias=True),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True)
        )

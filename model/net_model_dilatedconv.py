...
class DilatedConv(nn.Module):
    """Dilated Convolution followed by BatchNorm and ReLU."""

    def __init__(self, in_channels, out_channels, kernel_size=3, dilation=2, bias=False):
        """
        Initializes the DilatedConv module.

        Args:
            in_channels (int): Number of input channels.
            out_channels (int): Number of output channels.
            kernel_size (int, optional): Size of the convolutional kernel. Defaults to 3.
            dilation (int, optional): Dilation rate for the convolution. Defaults to 2.
            bias (bool, optional): If True, adds a learnable bias to the output. Defaults to False.
        """
        super(DilatedConv, self).__init__()
        padding = dilation  # To maintain the spatial dimensions
        self.conv = nn.Conv2d(
            in_channels,
            out_channels,
...
class DoubleConv(nn.Module):
    """(DilatedConv => [BN] => ReLU) * 2"""

    def __init__(self, in_channels, out_channels, mid_channels=None, dilation_rates=(2, 2)):
        """
        Initializes the DoubleConv module with dilated convolutions.

        Args:
            in_channels (int): Number of input channels.
            out_channels (int): Number of output channels.
            mid_channels (int, optional): Number of intermediate channels. Defaults to out_channels.
            dilation_rates (tuple, optional): Dilation rates for the two convolutional layers. Defaults to (2, 2).
        """
        super(DoubleConv, self).__init__()
        if not mid_channels:
            mid_channels = out_channels
        assert len(dilation_rates) == 2, "dilation_rates must be a tuple of two integers."

        self.double_conv = nn.Sequential(
            DilatedConv(in_channels, mid_channels, kernel_size=3, dilation=dilation_rates[0], bias=False),
            DilatedConv(mid_channels, out_channels, kernel_size=3, dilation=dilation_rates[1], bias=False)
        )
...
class Down(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, dilation_rates=(2, 2)):
        """
        Initializes the Down module with max pooling followed by DoubleConv.

        Args:
            in_channels (int): Number of input channels.
            out_channels (int): Number of output channels.
            dilation_rates (tuple, optional): Dilation rates for the DoubleConv layers. Defaults to (2, 2).
        """
        super(Down, self).__init__()
        self.maxpool_conv = nn.Sequential(
            nn.MaxPool2d(kernel_size=2),
            DoubleConv(in_channels, out_channels, dilation_rates=dilation_rates)
        )
...
class OutConv(nn.Module):
    """Output convolution layer with a standard 1x1 convolution."""

    def __init__(self, in_channels, out_channels):
        """
        Initializes the OutConv module with a 1x1 convolution.
...
class UNet(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True):
        ...
        self.inc = (DoubleConv(n_channels, 64, dilation_rates=(2, 2)))
        self.down1 = (Down(64, 128,dilation_rates=(2, 2)))
        self.down2 = (Down(128, 256,dilation_rates=(2, 2)))
        self.down3 = (Down(256, 512,dilation_rates=(2, 2)))
        factor = 2 if bilinear else 1
        self.down4 = (Down(512, 1024 // factor, dilation_rates=(2, 2)))

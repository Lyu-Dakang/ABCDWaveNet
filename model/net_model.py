...
from .shufflemixer_arch import FMBlock, FLLBlock
...
class WaveBottleNeck(nn.Module):
    def __init__(self, in_ch=64, n_lo_block=1):
        ...
        '''
        self.lo_blocks = nn.Sequential(*[FLLBlock(dim=in_ch, mlp_ratio=1.25) for _ in range(n_lo_block)])
        '''
...
class AdaptiveAttentionGate(nn.Module):
    def __init__(self, encoder_dim, global_dim, num_heads=8, top_percentage=1):
        super(AdaptiveAttentionGate, self).__init__()
        self.num_heads = num_heads
        self.top_percentage = top_percentage  # Percentage of top attention weights to keep (e.g., 0.1 for 10%)
        ...
        # Additional conv layer to match dimensions if necessary
    ...
    def forward(self, encoder_output, global_output):
        ...
        # Upsample global_output to match encoder_output spatial dimensions if needed
        if (H != Hg) or (W != Wg):
            global_output = F.interpolate(global_output, size=(H, W), mode='bilinear', align_corners=False)

        # Calculate query, key, and value and split into multiple heads
        query = self.query_conv(global_output).view(B, self.num_heads, self.head_dim, H, W)
        key = self.key_conv(encoder_output).view(B, self.num_heads, self.head_dim, H, W)
        value = self.value_conv(encoder_output).view(B, self.num_heads, self.head_dim, H, W)
        ...
        # Calculate the number of top elements to keep based on the percentage
        total_elements = attention_weights.size(-1)  # Total number of spatial locations (H*W)
        top_k_count = max(1, int(total_elements * self.top_percentage))  # Calculate Top-K based on percentage

        # Apply Top-K selection based on percentage
        topk_values, topk_indices = torch.topk(attention_weights, top_k_count, dim=-1)
        topk_mask = torch.zeros_like(attention_weights)
        topk_mask.scatter_(-1, topk_indices, topk_values)

        # Multiply attention weights by the Top-K mask
        attention_weights = attention_weights * topk_mask  # Keep only top-K weights

        # Normalize masked attention weights (optional step to ensure they sum to 1)
        ...
        # Add global output and apply LayerNorm
        ...
        # Compute gate weights and apply to attention output
        gate_weight = torch.sigmoid(self.gate(attention_output))  # (B, 1, H, W)
        gated_output = attention_output * gate_weight

        # Convert global_dim to encoder_dim if needed
        if self.output_conv is not None:
            gated_output = self.output_conv(gated_output)

        # Residual connection
        output = gated_output + encoder_output

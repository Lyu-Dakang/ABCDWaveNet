...
class UNet(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True):
        ...
        self.x4 = nn.Conv2d(256, 512, kernel_size=1)
        self.x3 = nn.ConvTranspose2d(256, 256, kernel_size=2, stride=2)
        self.x2 = nn.Conv2d(in_channels=256, out_channels=128, kernel_size=1)
        self.x1 = nn.Conv2d(256, 64,kernel_size=1)
    ...
    def forward(self, x):
        ...
        x2 = self.down1(x1)  # Downsample x1_dehaze to get x2
        #x2_freq = self.wavebottleneck2(x2)  # Dehaze x2 to get x2_dehaze
        #x2_dehaze = self.bnm2(x2_freq)

        x3 = self.down2(x2)  # Downsample x2_dehaze to get x3
        #x3_freq = self.wavebottleneck3(x3)  # Dehaze x3 to get x3_dehaze
        #x3_dehaze = self.bnm3(x3_freq)

        x4 = self.down3(x3)  # Downsample x3_dehaze to get x4
        #print(x4.shape)
        #x4_freq = self.wavebottleneck4(x4)  # Dehaze x4 to get x4_dehaze
        #x4_dehaze = self.bnm4(x4_freq)

        x5 = self.down4(x4)  # Downsample x4_dehaze to get x5
        #x5_freq = self.wavebottleneck5(x5)  # Dehaze x5 to get x5_dehaze
        #x5_dehaze = self.bnm5(x5_freq)

        global_output = self.global_info((x1, x2, x3, x4, x5))  # Global information aggregation
        #print(global_output.shape)
        #print(x1.shape, x2.shape, x3.shape, x4.shape, x5_dehaze.shape, global_output.size())

        d1 = self.up1(x5)
        x4_adjust = self.x4(global_output)
        #print(global_output.shape)
        #print(x4_adjust.shape)
        #x4_fusion = self.attention1(x4, global_output)
        x4_fusion = x4_adjust+x4
        d1 = x4_fusion + d1
        d1 = self.conv1(d1)

        d2 = self.up2(d1)
        x3_adjust =  F.interpolate(global_output, size=(64, 64), mode='bilinear', align_corners=False)
        #print(global_output.shape)
        #print(x3_adjust.shape)
        #x3_fusion = self.attention2(x3, global_output)
        #print(x3.shape)
        x3_fusion = x3+x3_adjust
        d2 = x3_fusion + d2
        d2 = self.conv2(d2)

        d3 = self.up3(d2)
        x2_adjust =self.x2(global_output)
        x2_adjust = F.interpolate(x2_adjust, size=(128, 128), mode='bilinear', align_corners=False)
        #print(global_output.shape)
        #print(x2_adjust.shape)
        #print(x2.shape)
        #x2_fusion = self.attention3(x2, global_output)
        x2_fusion = x2+x2_adjust
        d3 = x2_fusion + d3
        d3 = self.conv3(d3)

        d4 = self.up4(d3)
        #x1_fusion = self.attention4(x1, global_output)
        x1_adjust = self.x1(global_output)
        x1_adjust = F.interpolate(x1_adjust, size=(256, 256), mode='bilinear', align_corners=False)
        #print(global_output.shape)
        #print(x1_adjust.shape)
        #print(x1.shape)
        x1_fusion = x1+x1_adjust
        d4 = x1_fusion + d4
        d4 = self.conv4(d4)
        logits = self.outc(d4)
        return logits
